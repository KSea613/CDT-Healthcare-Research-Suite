...
from services.excel_service import load_excel, load_first_sheet_openpyxl, save_excel
import pandas as pd


# Defer PySide6 imports so this module can be imported in environments
# where Qt system libraries are not available (e.g., Codespaces).
try:
...
if _PYSIDE_AVAILABLE:
    class MainWindow(QMainWindow):
        ...
        def _create_toolbar(self):
            ...
            import_ministry_action = QAction("Import Ministry LTC Excel", self)
            import_ministry_action.triggered.connect(self.import_ministry_excel)
            toolbar.addAction(import_ministry_action)
        ...
        def import_ministry_excel(self):
            path, _ = QFileDialog.getOpenFileName(self, "Import Ministry LTC Excel", filter="Excel Files (*.xlsx)")
            if not path:
                return
            try:
                df = load_first_sheet_openpyxl(path)
                if df is None:
                    df = pd.DataFrame()
                self.df = df
                self._populate_table(df)
                self._set_status(f"Imported {len(df)} rows from ministry sheet")
    ...
    class MainWindow:
        def __init__(self, *args, **kwargs):
            raise RuntimeError("PySide6 is not available in this environment. Install PySide6 and system Qt libraries to use the GUI.")
